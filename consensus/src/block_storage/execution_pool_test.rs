...
fn get_roots(
    ...
    // TODO revisit, is block.id() suitable param for `block_to_commit_id`?
...
async fn create_block_tree_with_forks(
    ...
    // TODO revisit why does this contain genesis.block_info())
    let a3_r3 = inserter
        .insert_block(&a2_r2, 3, Some(genesis.block_info()))
        .await;
    let b1_r4 = inserter
...
/// Basically we want to ensure that the `window_root` is NEVER BEHIND the `commit_root`
...
async fn test_window_root_with_forks() {
    ...
    // TODO 1. fix brian's branch -- we don't want window_root to ever be none -- and 2. fix the diagram
    assert!(window_root.is_none());
...
/// TODO revisit if this formula should exclude the commit root and pruned nodes
#[tokio::test]
async fn test_window_root_with_non_sequential_round_forks() {
    ...
    // TODO why is the block window including the commit root...
    assert_eq!(block_window.len(), 3);
    assert_eq!(
        window_root.expect("Window root not found"),
        genesis_block.id()
    ...
    assert_eq!(block_window.first().unwrap().id(), a1_r1.id());
    // TODO why is the block window including pruned blocks...
    assert_eq!(block_window.len(), 3);
    assert_eq!(
        window_root.expect("Window root not found"),
        genesis_block.id()
