    ...
    fn print_metadata(&self, bytecode_path: &Path) -> Result<String, CliError> {
        ...
        metadata_map.insert(
            str::from_utf8(COMPILATION_METADATA_KEY)
                .expect("expect compilation_metadata")
                .to_string(),
            compilation_metadata_string,
        );
        metadata_map.insert(
            str::from_utf8(APTOS_METADATA_KEY_V1)
                .expect("expect aptos_metadata_key_v1")
                .to_string(),
        ...
        println!(
            "Metadata: {}",
            serde_json::to_string_pretty(&metadata_map).expect("expect metadata json string")
        );
        Ok("ok".to_string())
    }
