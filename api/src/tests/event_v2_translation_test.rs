...
static ACCOUNT_AND_COIN_MODULE_EVENT_MIGRATION: u64 = 81;

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_feature_enable_disable() {
    let mut context = new_test_context(current_function_name!());
    context
        .enable_feature(ACCOUNT_AND_COIN_MODULE_EVENT_MIGRATION)
        .await;
    assert!(
        context
            .is_feature_enabled(ACCOUNT_AND_COIN_MODULE_EVENT_MIGRATION)
            .await
    );
    context
        .disable_feature(ACCOUNT_AND_COIN_MODULE_EVENT_MIGRATION)
        .await;
    assert!(
        !context
            .is_feature_enabled(ACCOUNT_AND_COIN_MODULE_EVENT_MIGRATION)
...
async fn test_event_v2_translation_account_coin_register_event() {
    ...
    let payload = json!({
        ...
        "type_arguments": [],
        "arguments": [
            account2.address().to_hex_literal(), "102"
        ]
    });
    let resp = context.simulate_transaction(account1, payload, 200).await;
