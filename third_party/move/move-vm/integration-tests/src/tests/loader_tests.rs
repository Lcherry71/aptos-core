...
use move_vm_runtime::{
    ...
    storage::{
        ...
        module_storage::ModuleStorage, publishing::StagingModuleStorage,
    },
};
...
impl Adapter {
    fn new() -> Self {
        ...
        let store = InMemoryStorage::new(runtime_environment);
    ...
    fn publish_modules<'a, M: ModuleStorage>(
        &'a self,
        module_storage: &'a M,
    ...
    // TODO(loader_v2): Re-implement concurrent loading tests.
...
fn load() {
    ...
    let module_storage = adapter.storage().as_unsync_module_storage();

    let modules = get_modules();
    let module_storage = adapter.publish_modules(&module_storage, modules);

    adapter.call_functions(&module_storage);
}

// TODO(loader_v2): Re-activate tests below.
// #[test]
// fn load_concurrent() {
...
fn load_phantom_module() {
    ...
    let adapter = Adapter::new();
    let module_storage = adapter.storage().as_unsync_module_storage();
    let new_module_storage = adapter.publish_modules(&module_storage, modules);

    let mut session = adapter.vm.new_session(adapter.storage());
    let _ = session
...
fn deep_friend_list_ok_0() {
    ...
    let module_storage = adapter.storage().as_unsync_module_storage();
    let module_storage = adapter.publish_modules(&module_storage, modules);
    adapter.publish_modules(&module_storage, vec![module]);
}
