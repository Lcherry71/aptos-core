...
use move_binary_format::{
    ...
    file_format_common::{VERSION_DEFAULT, VERSION_MAX},
    CompiledModule,
};
use move_core_types::{
...
use move_vm_runtime::storage::{
    code_storage::CodeStorage,
    environment::RuntimeEnvironment,
    implementations::{
        unsync_code_storage::AsUnsyncCodeStorage, unsync_module_storage::AsUnsyncModuleStorage,
    },
...
fn make_script<'a>(dependencies: impl IntoIterator<Item = &'a str>) -> Vec<u8> {
    ...
    assert_ok!(script.serialize_for_version(Some(VERSION_MAX), &mut serialized_script));
    serialized_script
}
...
fn make_module<'a>(
    ...
    assert_ok!(module.serialize_for_version(Some(VERSION_MAX), &mut module_bytes));
...
fn test_deserialized_script_caching() {
    ...
    let mut module_bytes_storage = InMemoryStorage::new(runtime_environment);
...
fn test_module_does_not_exist() {
    let runtime_environment = RuntimeEnvironment::new(vec![]);
    let module_storage = InMemoryStorage::new(runtime_environment).into_unsync_module_storage();
...
fn test_deserialized_caching() {
    ...
    assert_some_eq!(assert_ok!(result), expected);
