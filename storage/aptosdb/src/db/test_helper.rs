...
pub(crate) fn update_store(
    ...
    for (i, (key, value)) in input.enumerate() {
        ...
        store
            .put_value_sets(
                &[WriteSet::new_for_test([(key.clone(), value.clone())])],
                version,
...
pub fn update_in_memory_state(state: &mut StateDelta, txns_to_commit: &[TransactionToCommit]) {
    ...
    for txn_to_commit in txns_to_commit {
        txn_to_commit.write_set.iter().for_each(|(key, op)| {
            state
                .updates_since_base
                .insert(key.clone(), op.as_state_value());
        });
        next_version += 1;
...
pub fn verify_committed_transactions(
    ...
    for txn_to_commit in txns_to_commit {
        ...
        for (state_key, write_op) in txn_to_commit.write_set().iter() {
